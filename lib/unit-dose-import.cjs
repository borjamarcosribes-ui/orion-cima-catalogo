...
const MIN_VALID_DETECTED_AT = new Date(Date.UTC(2000, 0, 1));
...
function normalizeDate(value) {
  ...
  if (value instanceof Date && !Number.isNaN(value.getTime())) {
    ...
    if (parsed) {
      parsedDate = new Date(Date.UTC(parsed.y, parsed.m - 1, parsed.d, parsed.H || 0, parsed.M || 0, Math.floor(parsed.S || 0)));
    }
  } else {
  ...
  if (!parsedDate || parsedDate < MIN_VALID_DETECTED_AT) {
    return null;
  }
