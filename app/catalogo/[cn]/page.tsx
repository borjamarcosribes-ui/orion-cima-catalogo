...
function renderDocLink(label: string, href: string | null): React.ReactNode {
  if (!href) {
    return <span className="muted">{label}: no disponible</span>;
  }

  return (
    <a href={href} target="_blank" rel="noreferrer" className="button button-ghost">
      {label}
    </a>
  );
}
...
export default async function CatalogDetailPage({ params }: PageProps) {
  ...
  return (
    <div className="grid" style={{ gap: 24 }}>
      ...
      <div>
        <Link href="/catalogo" className="button button-ghost">Volver al catálogo</Link>
      </div>
    </div>
  );
}
