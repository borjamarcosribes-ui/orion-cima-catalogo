...
import { executeScheduledNomenclatorUpdate } from '@/lib/nomenclator-update';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await runScheduledJob({
      ...
      handler: async () => executeScheduledNomenclatorUpdate(),
    });
    ...
    return NextResponse.json(
      {
        ok: false,
        error: error instanceof Error ? error.message : 'Error inesperado al ejecutar el job del Nomenclátor.',
      },
      { status: 500 },
    );
  }
