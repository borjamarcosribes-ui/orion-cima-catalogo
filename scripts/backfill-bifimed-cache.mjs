...
function extractFieldValueFromText(text, label, nextLabels = []) {
...
function mapFundingStatus(rawFundingModality) {
...
function buildSummary({ fundingModality, restrictedConditions, specialFundingConditions, nomenclatorState }) {
  const parts = [];

  if (fundingModality) {
    parts.push(fundingModality);
  }

  if (restrictedConditions) {
    parts.push(`Restricciones: ${restrictedConditions}`);
  }

  if (specialFundingConditions) {
    parts.push(`Condiciones especiales: ${specialFundingConditions}`);
  }

  if (nomenclatorState) {
    parts.push(`Estado nomenclátor: ${nomenclatorState}`);
  }
...
function extractSummaryFieldsFromTables(html) {
...
function extractIndicationsFromTables(html) {
  const tables = extractRowsFromTables(html);
...
function extractIndicationsFromLinearText(text) {
...
function parseBifimedHtml(html, cn) {
...
async function fetchBifimedHtmlByCn(cn) {
...
async function upsertBifimedCache(summaryRow, indications) {
...
async function backfillBifimedCache(options) {
  ...
  for (const cn of targetCns) {
    try {
      ...
      if (!html) {
        summary.notFound += 1;
        console.log(`[BIFIMED] ${cn} -> sin respuesta`);
      } else {
        const parsed = parseBifimedHtml(html, cn);

        if (!parsed) {
          summary.notFound += 1;
          console.log(`[BIFIMED] ${cn} -> sin detalle utilizable`);
        } else {
          await upsertBifimedCache(parsed.summaryRow, parsed.indications);
          summary.updated += 1;
          console.log(
            `[BIFIMED] ${cn} -> cache actualizado (${parsed.indications.length} indicación${
              parsed.indications.length === 1 ? '' : 'es'
            })`,
          );
        }
      }
      ...
      console.error(`[BIFIMED] ${cn} -> ERROR: ${message}`);
    }
...
async function main() {
  ...
  console.log('Backfill BIFIMED iniciado con opciones:', options);
  console.log('Base URL BIFIMED:', BIFIMED_BASE_URL);

  const summary = await backfillBifimedCache(options);
